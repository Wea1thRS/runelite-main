...
public class OneClickPlugin extends Plugin
{
		...
		switch (type)
		{
			...
			case MEDPACK:
				if (opcode == MenuOpcode.NPC_FIRST_OPTION.getId() &&
						event.getTarget().equals("<col=ffff00>Wounded soldier"))
				{
					...
					event.setOption("Use");
					event.setTarget("<col=ff9040>Shayzien medpack<col=ffffff> -> <col=ffff00>Wounded soldier");
					event.setForceLeftClick(true);
					event.setModified();
				}
				break;
	...
	private void onMenuOptionClicked(MenuOptionClicked event)
	{
		...
		switch (type)
		{
			...
			case MEDPACK:
				if (opcode == MenuOpcode.NPC_FIRST_OPTION.getId() &&
					target.equals("<col=ff9040>Shayzien medpack<col=ffffff> -> <col=ffff00>Wounded soldier"))
				{
